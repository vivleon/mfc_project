...
{
	// [수정] 메모리 누수 방지를 위해 콤보박스 ItemDataPtr 메모리 해제
	// OnBnClickedOk가 아닌 소멸자에서 처리해야 취소/닫기 시에도 해제됩니다.
	try {
...
BOOL CCameraSettingsDlg::OnInitDialog()
{
	...
	m_tabSettings.InsertItem(1, _T("고급 설정(비활성화)"));
...
void CCameraSettingsDlg::OnBnClickedOk()
{
	...
	// [수정] 메모리 누수 방지를 위해 CString 정리 코드를 여기에서 제거하고
		// 소멸자로 이동시킵니다.
